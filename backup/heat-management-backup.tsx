...
import { Label } from "@/components/ui/label";      // Actualizar en la base de datos
      for (const update of       // Resetear heats y pistas de los equipos
      try {
        ...
        // Intentar detectar si la columna lane existe
        try {
      ...
      }eateClient } from "@/lib/supabase/client";
...
export function HeatManagement({ teams, workouts }: HeatManagementProps) {
    ...
    try {
      ...
      for (const update of updates) {
        const { error } = await supabase
          ...
          .update({
            heat: update.heat,
            // lane: update.lane // Comentado temporalmente hasta agregar columna
          })
          .eq("id", update.id);
  ...
  const resetHeats = async () => {
    ...
    try {
      // Resetear heats y pistas de los equipos
      const { error: teamsError } = await supabase
        .from("teams")
        .update({
          heat: null,
    ...
    <div className="space-y-4">
      <div className="bg-orange-50 p-4 rounded-lg">
        <h3 className="font-semibold text-orange-900 mb-2">
          Información de Heats
        </h3>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
          <div>
            <span className="text-gray-600">Total Equipos:</span>
            <div className="font-bold text-orange-900">{stats.totalTeams}</div>
          </div>
          <div>
          ...
          <div>
            <span className="text-gray-600">Total Heats:</span>
            <div className="font-bold text-orange-900">{stats.totalHeats}</div>
          </div>
          <div>
                    ...
                    >
                      <div className="flex justify-between items-start mb-2">
                        ...
                        <span className="text-xs text-gray-500">
                          {heat.teams.length} equipo
                          {heat.teams.length !== 1 ? "s" : ""}
                        </span>
                      </div>
                      ...
                      <div className="flex flex-wrap gap-1">
                        {heat.teams.map((team) => (
                          <span
                            key={team.id}
                            className="bg-white border border-orange-200 text-orange-700 text-xs px-2 py-1 rounded flex items-center space-x-1"
                          >
                            <span>{team.name}</span>
                            {team.lane && (
